/* NTM: I used Arduino > Tools > Manage Libraries to install the library
  ...
  After installing, I used the example given in
...
void setup()
{
  ...
  if ( imu.accelAvailable() )
  {
    ...
    mag_a_0 = sqrt(ax * ax + ay * ay + az * az);
    t0 = millis();
    old_slope = 0;
...
void loop()
{
  if ( imu.accelAvailable() )
  {
    ...
    t1 = millis();
    slope_a = (mag_a - mag_a_0) / (t1 - t0);
    if (slope_a < 0 && old_slope > 0 && mag_a > 1.2 && (millis() - time_of_last_peak) > 100) {
      // found a peak, yay!
      num_steps++;
      time_of_last_peak = millis();
    }
